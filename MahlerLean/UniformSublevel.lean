import MahlerLean.UniformSublevelIntervals
...
/-- Uniform analytic sublevel theorem: positive constants, a smallness
threshold in `(0,1)`, an exact finite interval union, and the common power
measure estimate. This is the analytic case needed by the manuscript. -/
theorem exists_uniform_analytic_sublevel_bound
  ...
  obtain ⟨lamR, hlamR, R, hR⟩ :=
    exists_uniform_analytic_sublevel_interval_cover (by omega) φ hU hφ hJU hW
  let p : ℝ := (((N - 1 : ℕ) : ℝ)⁻¹)
  ...
  · intro c hc eps heps hsmall
    ...
    constructor
    · obtain ⟨t,ht,hconn,hcov⟩ := hR c hc eps heR
      exact ⟨t, by omega, hconn, hcov⟩
    · have hm := hM c hc eps heps heM
      have hp : (eps / lamM) ^ p = eps ^ p / lamM ^ p :=
        Real.div_rpow heps hlamM.le p
      have hm' : volume.real {x | x ∈ Icc A B ∧ ‖jetLinearCombo φ c x‖ ≤ eps}
          ≤ D * eps ^ p := by
...
/-- Rational-relation specialization for the coefficient vector in
lexicographic order `(0,0),(0,1),...,(d,1)`. The constants precede the
unit coefficient vector and the sublevel height in the quantifiers. -/
theorem exists_uniform_rational_relation_sublevel_bound
    ...
    ∃ eps0 : ℝ, 0 < eps0 ∧ eps0 < 1 ∧
      ...
      ∀ eps : ℝ, 0 ≤ eps → eps < eps0 →
        (∃ t : Finset (Set ℝ), t.card ≤ R ∧
          (∀ E ∈ t, E.OrdConnected) ∧
          (∀ x, (x ∈ Icc A B ∧
            ‖∑ j : Fin (2 * (d + 1)), c j * (x ^ (j.val / 2) * f x ^ (j.val % 2))‖ ≤ eps) ↔
              ∃ E ∈ t, x ∈ E)) ∧
        volume.real {x | x ∈ Icc A B ∧
          ‖∑ j : Fin (2 * (d + 1)), c j * (x ^ (j.val / 2) * f x ^ (j.val % 2))‖ ≤ eps}
          ≤ C * eps ^ (((2 * (d + 1) - 1 : ℕ) : ℝ)⁻¹) := by
  simpa only [jetLinearCombo, rationalFamily] using
    exists_uniform_analytic_sublevel_bound (by omega : 2 ≤ 2 * (d + 1))
